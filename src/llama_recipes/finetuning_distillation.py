...
def main(**kwargs):
    ...
    dataset_train = SupervisedDataset(train_config.dataset, tokenizer, index=True, seq_len=False)
    if train_config.run_validation and train_config.val_ds:
        dataset_val = SupervisedDataset(train_config.val_ds, tokenizer, index=True, seq_len=False)
    ...
    distillation_mode = os.getenv('DISTILLATION_MODE')
    if distillation_mode == 'train':
        results = train(
            model,
            train_dataloader,
            eval_dataloader,
        ...
        raise(f'DISTILLATION_MODE必须设置为train或eval,train用于训练,eval用于离线计算hidden states,当前DISTILLATION_MODE={DISTILLATION_MODE}')
    if distillation_mode=='tran' and (not train_config.enable_fsdp or rank==0):
